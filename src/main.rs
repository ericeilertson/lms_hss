...
fn main() {
    ...
    match cli.command {
        Commands::CreateTree { 
            ...
            match create_tree_command(&public_key_file, &private_tree_file, &lms_height, hash_width, ots_w) {
...
fn create_tree_inner<const N: usize>(
    ...
    let lms_type = match (N, lms_height) {
        ...
        _ => return Err(format!("Invalid combination: hash_width={}, height={}", N, lms_height)),
    };
    ...
    let ots_type = match (N, ots_w) {
        ...
        _ => return Err(format!("Invalid combination: hash_width={}, ots_w={}", N, ots_w)),
    };
...
fn sign_inner<const N: usize>(
    ...
    if q_to_use as usize >= tree.private_keys.len() {
        return Err(format!("q value {} is out of range (max: {})", q_to_use, tree.private_keys.len() - 1));
    }
...
impl PrivateTreeData {
    fn from_tree<const N: usize>(
        ...
        PrivateTreeData {
            ...
            q: tree.q,
            t_tree: tree.t_tree.iter().map(|h| hex::encode(h.as_ref())).collect(),
            private_keys: tree.private_keys.iter()
    ...
    fn to_tree<const N: usize>(&self) -> Result<lms_hss::LmsTree<N>, String> {
        ...
        let t_tree: Result<Vec<_>, _> = self.t_tree.iter()
            .map(|hex_str| {
                let bytes = hex::decode(hex_str)
                    .map_err(|_| "Failed to decode hex in t_tree")?;
                if bytes.len() != N {
        ...
        let private_keys: Result<Vec<Vec<_>>, _> = self.private_keys.iter()
            .map(|key_set| {
                key_set.iter()
                    .map(|hex_str| {
                        let bytes = hex::decode(hex_str)
