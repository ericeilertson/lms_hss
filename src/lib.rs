...
mod tests {
    ...
    fn test_get_lms_parameters() {
        ...
        assert_eq!(n, 24);
        assert_eq!(h, 25);
        assert!(get_lms_parameters(&LmsAlgorithmType::LmsReserved).is_err());
    }
    ...
    fn test_parse_signature() {
        let signature = [
            0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0xd5, 0x7d, 0x37, 0xf5, 0xe4, 0x6b,
            0xfb, 0x75, 0x20, 0x80, 0x6b, 0x07, 0xa1, 0xb8, 0x50, 0x65, 0x0e, 0x3b, 0x31, 0xfe,
            0x4a, 0x77, 0x3e, 0xa2, 0x9a, 0x07, 0x1a, 0x00, 0x00, 0x00, 0x01, 0x00,
        ];

        let result = parse_signature_contents::<24>(&signature[0..4]);
        assert_eq!(result, Err("Signature string is too short".to_string()));

        let result = parse_signature_contents::<24>(&signature);
        assert_eq!(
            result,
            Err("LMOTS hash width does not match the const parameter provided".to_string())
        );

        let result = parse_signature_contents::<32>(&signature);
        assert_eq!(result, Err("Signature string is too short".to_string()));
    }
    ...
    fn test_parse_public_key() {
        let public_key_bytes = [
            0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x07, 0xe4, 0x6b, 0xfb, 0x75, 0x20, 0x80,
            0x6b, 0x07, 0xa1, 0xb8, 0x50, 0x65, 0x0e, 0x3b, 0x31, 0xfe, 0x4a, 0x77, 0x3e, 0xa2,
            0x9a, 0x07, 0x1a, 0x00, 0x23, 0x00, 0x00, 0x00, 0x05, 0xd5, 0x7d, 0x37, 0xf5, 0xe4,
            0x6b, 0xfb, 0x75, 0x20, 0x80, 0x6b,
        ];
        let result = parse_public_contents::<24>(&public_key_bytes);
        assert!(result.is_ok());

        let result = parse_public_contents::<24>(&public_key_bytes[0..47]);
        assert_eq!(
            result,
            Err("Public key string is the wrong size".to_string())
        );

        let result = parse_public_contents::<32>(&public_key_bytes[0..48]);
        assert_eq!(
            result,
            Err("Public key string is the wrong size".to_string())
        );

        let result = parse_public_contents::<23>(&public_key_bytes[0..47]);
        assert_eq!(
            result,
            Err(
                "Hash width specified in the LMS type does not match the const N provided"
                    .to_string()
            )
        );
    }
    ...
    #[test]
    fn hss_upper() {
        const IDENTIFIER: LmsIdentifier = [
            0x61, 0xa5, 0xd5, 0x7d, 0x37, 0xf5, 0xe4, 0x6b, 0xfb, 0x75, 0x20, 0x80, 0x6b, 0x07,
            0xa1, 0xb8,
        ];
        const HSS_PUBLIC_KEY: HashValue<32> = HashValue([
        ...
        const LMS_PUBLIC_KEY: LmsPublicKey<32> = LmsPublicKey {
    ...
    fn lms_lower() {
        ...
        const LMS_PUBLIC_HASH: HashValue<32> = HashValue([
            0x6c, 0x50, 0x04, 0x91, 0x7d, 0xa6, 0xea, 0xfe, 0x4d, 0x9e, 0xf6, 0xc6, 0x40, 0x7b,
            0x3d, 0xb0, 0xe5, 0x48, 0x5b, 0x12, 0x2d, 0x9e, 0xbe, 0x15, 0xcd, 0xa9, 0x3c, 0xfe,
            0xc5, 0x82, 0xd7, 0xab,
        ]);
        ...
        const FINAL_PATH: [Sha256Digest; 5] = [
            HashValue([
    ...
    fn lms_failure() {
        ...
        const LMS_PUBLIC_IDENTIFIER: LmsIdentifier = [
